...
# Lista de excepciones (añade más cuando necesites)
NOMBRES_EXCEPCION = {
    "Ἰεσσαί": "[Iessaí]",
    "Μωυσῆς": "[Moisés]",
    "Ἐλισσαίε": "[Elissaie]",
...
# normalizamos las claves para búsquedas rápidas
#NOMBRES_EXCEPCION_NORM = {unicodedata.normalize('NFC', k): v for k, v in NOMBRES_EXCEPCION.items(
...
def replace_diptongos(text):
    """
    Detecta diptongos y el caso especial 'α...υ...ι...' (ej. Δαυὶδ).
    Trabaja sobre texto normalizado a NFD.
    """
    def repl_auv_i(m):
        seg = m.group(0)
        # Si la iota tiene acento (combining OR precompuesto), devuelve 'aví'
        if re.search(r'ι[\u0300-\u036f]*[\u0301\u0300\u0342]', seg) or re.search(r'[\u03AF\u1F76\u1FD]', seg):
            return 'aví'
        return 'avi'

    # Caso especial antes de reglas normales
    text = re.sub(r'α[\u0300-\u036f]*υ[\u0300-\u036f]*ι[\u0300-\u036f]*', repl_auv_i, text)

    # Resto de diptongos normales
    ...
    text = re.sub(r'Ο[\u0300-\u036f]*υ', 'U', text)
    text = re.sub(r'α[\u0300-\u036f]*υ', 'au', text) #cambio de af
    text = re.sub(r'Α[\u0300-\u036f]*υ', 'Au', text) #cambio de Af
...
def postprocess_gamma(text):
    ...
    text = text.replace("Ge", "Ye").replace("Gi", "Yi")
    return text
def postprocess_av_ef(text):
    """
    Ajusta 'au' y 'eu' a av/af o ev/ef dependiendo de la consonante siguiente,
...
def transliterate_modern_greek_v20(text_koine):
    if text_koine in NOMBRES_EXCEPCION:
        return NOMBRES_EXCEPCION[text_koine]
    ...
    translit = postprocess_av_ef(translit)
